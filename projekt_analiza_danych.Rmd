---
title: "Wpływ spożycia alkoholu na funkcje poznawcze studentów uczelni wyższych"
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    btn.addEventListener("click", function() {
        ...
        btn.innerHTML = toc.classList.contains("hidden") ? "🡲" : "☰";
    });

    // 4. OBSŁUGA STRZAŁEK
    // Wewnątrz .toc-inner-container
    ...
    itemsWithChildren.forEach(function(li) {
        ...
        arrow.addEventListener("click", function(e) {
            e.stopPropagation(); // Nie uruchamiaj linku, tylko strzałkę
...
Przed przystąpieniem do uzupełniania danych, przeprowadzono wizualną inspekcję brakujących wartości przy użyciu pakietów `naniar`. Pozwoliło to na podjęcie kluczowych decyzji:
...
ggplot(dane_rain, aes(x = sex, y = GPA, fill = sex)) +
  ...
  scale_fill_manual(values = c("Kobieta" = "deeppink3", "Mężczyzna" = "#003357")) +
  scale_color_manual(values = c("Kobieta" = "deeppink3", "Mężczyzna" = "#003357")) +
...
```{r spożycie alkoholu ilustracja, echo=FALSE, warning=FALSE, message=FALSE, fig.height=3.5, out.width="100%"}
dane_piktogram <- dane %>%
  filter(!is.na(drinks)) %>%
  count(drinks) %>%
  mutate(pct = n / sum(n),
...
```{r różnice między wydziałami, echo=FALSE, fig.align='center'}
dane_fac <- dane %>%
...
```{r porównanie czy warto sie uczyc, echo=FALSE, fig.align='center'}
dane_nauka <- dane %>%
  mutate(GPA = ifelse(is.na(grade_last_y), grade_12, grade_last_y)) %>%
...
Wykres przedstawia porównanie wyników GPA studentów w zależności od czasu poświęcanego na naukę własną: grupa 1 to do 5 godzin tygodniowo (n = 206), natomiat grupa 2: powyżej 5 godzin tygodniowo (n = 192).
...
```{r czy nauka osłabia negatywny wpływ imprezowania, echo=FALSE, fig.align='center'}
dane_int <- dane %>%
  mutate(GPA = ifelse(is.na(grade_last_y), grade_12, grade_last_y)) %>%
  filter(!is.na(partying), !is.na(studying), !is.na(GPA)) %>%
  mutate(study_level = ifelse(studying %in% c("0", "1-3", "3-5"), "Mało nauki (<5h)", "Dużo nauki (>5h)")) %>%
