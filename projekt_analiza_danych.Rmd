...
```{r shiny nie wiadomo co z tym zrobić, eval=FALSE, include=FALSE}
dane_clust <- dane %>%
  mutate(
...
ui <- fluidPage(
  ...
  titlePanel("🔥 Interaktywna Mapa Archetypów Studentów"),

  sidebarLayout(
    sidebarPanel(
      sliderInput("clusters", "Liczba archetypów:", 
                  min = 2, max = 6, value = 4, step = 1),
      helpText("Archetypy mogą być np.: Imprezowicz, Robot, Przeciętniak, Ryzykant itd.")
    ),
...
server <- function(input, output, session){
  ...
  output$archetypeDescriptions <- renderUI({
    ...
    desc <- c(
      "1" = "<b>Archetyp 1:</b> Imprezowicz – dużo imprezuje, dużo pije, nisko się uczy.",
      "2" = "<b>Archetyp 2:</b> Robot – dużo się uczy, mało imprezuje, dobre oceny.",
      "3" = "<b>Archetyp 3:</b> Przeciętniak – średnio we wszystkim.",
      "4" = "<b>Archetyp 4:</b> Ryzykant – dużo imprezuje, ale oceny jeszcze trzymają się średnio.",
      "5" = "<b>Archetyp 5:</b> Samozagłada 
      ,3000
      – dużo imprez + mało nauki + fatalne oceny.",
      "6" = "<b>Archetyp 6:</b> Ukryty Robot – mało imprezuje, oceny wysokie mimo małego nakładu pracy."
    )
  ...
  output$pcaPlot <- renderPlotly({
    ...
    p <- ggplot(df, aes(PC1, PC2, color = cluster)) +
      ...
      labs(title = "Mapa Archetypów Studentów (PCA + clustering)",
...
dane_analiza <- dane %>%
  mutate(
    # Konwersja allowance na wartość numeryczną (środek przedziału)
    allow_val = case_when(
    ...
    # Grupowanie dochodu dla lepszej czytelności (Niski vs Wysoki)
...
# Tworzymy zmienną hybrydową GPA dla pełniejszego obrazu
dane_desc <- dane %>%
...
# 3. Profil stylu życia (Studying vs Partying)
...
ggplot(dane_rain, aes(x = sex, y = GPA, fill = sex)) +
  ...
  scale_color_tq() +
  labs(
    title = "Rozkład ocen względem płci",
    x = "Płeć", y = "Wynik akademicki (%)"
  ) +
  coord_flip() + # Obrócenie dla lepszego efektu "chmury"
  theme_tq() +
...
datasummary_balance(~Stypendium, 
                   data = dane_tab,
                   title = "Charakterystyka grup względem statusu stypendialnego",
                   dinm = FALSE) # Wyłącza różnicę średnich dla uproszczenia
...
dane_analiza <- dane %>%
  mutate(
    ...
    party_num_score = case_when(
      ...
      partying == "2" ~ 2,
      partying == "3" ~ 3,
      partying == "4+" ~ 4, # lub 5, w zależności jak ustaliłeś w modelu logitowym
      TRUE ~ NA_real_
    ),
...
# 3. Profesjonalna tabela wyników testu
tidy(chi_test) %>%
  select(`Statystyka Chi2` = statistic, `df` = parameter, `p-value` = p.value) %>%
  kable(digits = 4, caption = "Wyniki testu istotności Chi-kwadrat")
```
...
```{r manova ancova anova (do obczajenia), echo=FALSE}
# 1. Przygotowanie danych (Faktoryzacja)
dane_ancova <- dane %>%
  mutate(
    GPA = ifelse(is.na(grade_last_y), grade_12, grade_last_y),
    study_num = case_when(
    ...
    faculty = as.factor(faculty) # Kluczowe dla stabilności
  ) %>%
...
# --- TABELA 1: ISTOTNOŚĆ (ANOVA Typ III) ---
Anova(model_ancova, type = "III") %>%
  tidy() %>%
  filter(term != "(Intercept)") %>%
  mutate(p.value = scales::pvalue(p.value)) %>%
  rename(Czynnik = term, `Suma kwadratów` = sumsq, `Statystyka F` = statistic, `p-value` = p.value) %>%
  kable(digits = 3, booktabs = TRUE, caption = "Tabela 1: Testy istotności efektów głównych") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F, position = "center") %>%
  row_spec(0, bold = TRUE, color = "white", background = "#446e9b")

# --- WYKRES 1: WIZUALIZACJA WPŁYWU (Forest Plot) ---
# To jest "ładniejsza" alternatywa dla tabeli współczynników
plot_model(model_ancova, 
           type = "est", 
           show.values = TRUE, 
           value.offset = .3,
           title = "Wpływ predyktorów na GPA (%)",
           axis.labels = NULL, # Automatycznie pobierze z modelu
           vline.color = "red",
           dot.size = 3,
           line.size = 1,
           colors = "Set1") +
  theme_minimal() +
  labs(subtitle = "Współczynniki regresji z przedziałami ufności 95%")
```
